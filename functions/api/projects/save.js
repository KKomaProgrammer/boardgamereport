...
export async function onRequest(context) {
  ...
  if (!['video_workspace','ppt_workspace','boardgame_workspace'].includes(body.workspaceType)) throw Object.assign(new Error('workspaceType 값이 올바르지 않습니다.'), { status: 400 });
  ...
  const { error } = await supabase.from('project_documents').upsert({ project_id: body.projectId, document_type: body.workspaceType, content_enc, updated_by: user.id, updated_at: new Date().toISOString() }, { onConflict: 'project_id,document_type' });
  if (error) throw error;
  const mode = ['video','ppt','boardgame'].includes(body.mode) ? body.mode : 'ppt';
  await supabase.from('projects').update({ current_mode: mode, updated_at: new Date().toISOString() }).eq('id', body.projectId);
