...
export async function onRequest(context) {
  ...
  if (!['video_workspace','ppt_workspace','boardgame_workspace'].includes(body.workspaceType)) throw Object.assign(new Error('workspaceType이 올바르지 않습니다.'), { status: 400 });
  ...
  await supabase.from('projects').update({ current_mode: body.mode || undefined, updated_at: new Date().toISOString() }).eq('id', body.projectId);
  if (body.makeVersion) {
    await supabase.from('project_versions').insert({ project_id: body.projectId, document_type: body.workspaceType, content_enc, created_by: user.id, summary: body.summary || '자동 저장 버전' });
  }
