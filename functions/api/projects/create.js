...
export async function onRequest(context) {
  ...
  const { data: project, error } = await supabase.from('projects').insert({
    title: cleanText(body.title, 120), description: cleanText(body.description, 1000), owner_id: user.id, current_mode: mode
  }).select('*').single();
